<script lang="ts">
	...
	import { Plus, Eye, Pencil, Trash2, Key, Power } from "@lucide/svelte";
	...
	function filterData() {
		...
		filtered = dosen.filter(
			(d) =>
				d.name.toLowerCase().includes(key) ||
				d.email.toLowerCase().includes(key) ||
				d.nidn.toLowerCase().includes(key),
		);

		currentPage = 1;
	}

	const totalPages = $derived.by(() => Math.ceil(filtered.length / perPage));
	...
	function detailDosen(data: Dosen) {
		selected = data;

		showDetail = true;
	}

	function hapusDosen(data: Dosen) {
		selected = data;
	...
	async function handleResetPassword(userId: number, name: string) {
		...
		if (result.isConfirmed) {
			try {
				...
				Swal.fire(
					"Gagal!",
					err?.response?.data?.message || "Gagal me-reset password.",
					"error",
				);
			}
		}
	...
	async function handleToggleStatus(
		...
		if (result.isConfirmed) {
			try {
				...
				Swal.fire(
					"Berhasil!",
					`Status akun ${name} berhasil diubah.`,
					"success",
				);
				loadDosen();
			} catch (err: any) {
...
<div class="space-y-6">
	...
	>
		<div>
			...
			<p class="text-slate-500">Kelola seluruh data dosen.</p>
		</div>

		<Button onclick={tambahDosen}>
			<Plus size={18}></Plus>
	...
	{#if loading}
		<div class="flex justify-center py-20">
			<LoadingSpinner text="Memuat data dosen..." />
		</div>
